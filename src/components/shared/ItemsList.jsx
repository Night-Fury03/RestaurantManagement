...
export default function ItemsList({ data }) {
    ...
    return (
        <div className='flex-1 flex flex-row flex-wrap w-full py-2 gap-6 max-h-[400px] overflow-y-scroll scrollbar-none'>
            <div>
                <Button onClick={showModal} className='flex flex-col rounded-2xl w-48 h-full border border-dashed border-customPrimary hover:!bg-transparent bg-customDark1 text-customPrimary'>
                    <PlusOutlined />
                    Thêm món mới
                </Button>
            ...
            {
                data.map((item, index) => (
                    <div key={index} className='flex flex-col rounded-2xl items-center border border-customDarkLine w-48'>
                        ...
                        >
                            <Button className='relative group overflow-hidden w-full py-6 rounded-b-2xl rounded-t-none border-none hover:!bg-customPrimaryOpacity bg-customPrimaryOpacity text-customPrimary font-semibold active:translate-y-[2px] active:brightness-90'>
                                <span class="absolute bottom-0 right-0 w-full h-full transition-all duration-500 ease-in-out delay-200 translate-y-full bg-customPrimaryOpacity_50 rounded-md group-hover:translate-y-0"></span>
                                <span class="relative w-full gap-x-2 justify-center text-white"><DeleteOutlined /> Xóa</span>
                            </Button>
                        </Popconfirm>
                    </div>
                ))
