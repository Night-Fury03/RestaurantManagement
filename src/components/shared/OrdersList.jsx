      ...
      <div className="flex flex-row flex-wrap gap-8">
        {data.area.table.map((item, index) => (
          <div
            ...
            className="flex flex-col items-center justify-center min-w-56 p-4 bg-white gap-y-2 rounded transition-all duration-300 hover:shadow-[0_0_15px_rgba(59,130,246,0.8)] hover:scale-105 cursor-pointer"
          >
            <div className="flex-1 pb-2 flex justify-center items-center w-full border-b border-customDarkLine ">
              <h1>
                {data.area.name}.{item.id}
              </h1>
            </div>
            <div className="flex justify-between text-xs w-full gap-x-2 ">
              ...
              >
                {item.isActive ? "hoat dong" : "chua hoat dong"}
              </span>
            </div>
          </div>
        ))}
        ...
        <Modal
          title={
            currentTable.isActive
              ? `Bàn ${currentTable.id} - Hoạt động`
              : `Bàn ${currentTable.id} - Chưa hoạt động`
          }
          open={isModalOpen}
          onOk={handleOk}
