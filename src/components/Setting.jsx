...
export default function Setting() {
  ...
  const [modalEditCategoryVisible, setModalEditCategoryVisible] =
    useState(false);
    ...
    formEditCategory
      ...
      .then(async (values) => {
        ...
        try {
          // Gửi dữ liệu lên server
          await axios.put(
            `https://localhost:7215/food/categories/${item.id}`,
            { id: item.id, isHidden: 0, ...values },
            {
              headers: { "Content-Type": "application/json" },
            }
          );
    ...
    formEditAcc
      ...
      .then(async (values) => {
        ...
        try {
          // Gửi dữ liệu lên server
          await axios.put(
            `https://localhost:7215/Account/${item.userName}`,
            { type: 0, isHidden: 0, ...values },
            {
              headers: { "Content-Type": "application/json" },
            }
          );
            ...
            <div className="flex flex-col gap-y-2 max-h-[340px] overflow-y-scroll">
              {foodCategories.map((category) => (
                ...
                >
                  ...
                  <div>
                    <Button
                      onClick={() => showModalEditCategory(category)}
                      type="text"
                      icon={<EditOutlined />}
                      className="hover:!bg-transparent w-12 hover:scale-125 hover:!text-customSecondary"
                    >
                      {" "}
                    </Button>
                    <Popconfirm
                      ...
                      onConfirm={async () => {
                        await axios.delete(
                          `https://localhost:7215/food/categories/${category.id}`
                        );
                        getFoodCategoriesApi();
                      }}
                    ...
                    >
                      <Button
                        ...
                        danger
                        icon={<DeleteOutlined />}
                        className="text-customPrimary hover:!bg-transparent w-12 hover:scale-125 hover:!text-customPrimary"
                      />
                    </Popconfirm>
                  </div>
                </div>
